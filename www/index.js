    ...
    return new IntersectionObserver((entries) => {
      for (const i of entries) {
        if (i.isIntersecting) {
          ...
          if (
            this.article.offsetHeight - i.boundingClientRect.bottom <= 0
          ) {
            this.summary
              .querySelector(`a[href="#${i.target.id}"]`)
              ?.classList.remove('active');

            if (this.summary.getElementsByClassName('active').length <= 0) {
              this.summary
                .querySelector(`a[href="#${i.target.id}"]`)
            ...
            i.boundingClientRect.y - this.article.offsetHeight < 0
            && this.summary.getElementsByClassName('active').length > 1
          ) {
            this.summary
